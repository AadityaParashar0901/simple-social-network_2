<?php
    ...
    if (isset($_REQUEST["register_button"]) && $_REQUEST["register_button"] == "true") {
        ...
        $query = "select EMail from users where Username = '$username';"; echo $query; $result = $connection->query($query);
        if ($result->num_rows > 0) {
            $_SESSION["toast_message"] = "Username already exists";
            header("Location: index.php");
        }
        $query = "insert into users(Username, EMail, Password) values ('$username', '$email', '$password');"; echo $query; $result = $connection->query($query);
        if ($result === true) {
